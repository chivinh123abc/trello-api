    ...
    const result = await GET_DB().collection(BOARD_COLLECTION_NAME).aggregate([
      ...
      },
      {
        $lookup: {
          ...
          foreignField: '_id',
          as: 'owners',
          // pipline trong lookup xu ly hoac nhieu luong can thiet
          // $project de chi dinh cac field k muon lay = 0
...
const getBoards = async (userId, page, itemsPerPage, queryFilters) => {
  try {
    ...
    // Xu ly query filter cho tung truong hop search board, vd search theo title
    if (queryFilters) {
      // console.log(Object.keys(queryFilters))
      Object.keys(queryFilters).forEach(key => {
        // Phan biet chua hoa chu thuong
